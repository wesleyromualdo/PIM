<?php 
require APPRAIZ . 'obras2/includes/principal/listaExecFinanceira/funcoes.php';
...
switch ($_REQUEST['requisicao']) {
    ...
    case "excluirMedicao":
        ...
        if (!verificaSeTemNotaFiscal($medid)) {
            if (excluirMedicao($medid)) {
                echo "Medição excluída com sucesso.";
                exit;
            }
            echo "Falha ao excluir a medição";
            exit;
        } else {
            echo "Não é possível excluir a medição, pois ela já possui registro em nota fiscal.";
            exit;
        }
        break;
}
