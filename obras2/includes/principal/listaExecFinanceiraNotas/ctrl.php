<?php
require APPRAIZ . 'obras2/includes/principal/listaExecFinanceiraNotas/funcoes.php';
...
switch ($_REQUEST['requisicao']) {
    ...
    case 'excluirNotaFiscal':
        ...
        if(!verificaSeTemPagamento($ntfid)) {
            if (excluirNotaFiscal($ntfid)) {
                echo "Nota Fiscal excluída com sucesso.";
                exit;
            }
            echo "Falha ao excluir a Nota Fiscal";
            exit;
        }
        else {
            echo "Não é possível excluir a Nota Fiscal, pois ela já possui registro de pagamento.";
            exit;
        }
        break;
    case "download":
