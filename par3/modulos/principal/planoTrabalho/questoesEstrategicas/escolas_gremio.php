...
$qrpid = (int) $_POST['qrpid'];
$perid = (int) $_POST['perid'];
$inuid = (int) $_GET['inuid'];

if ($qrpid && $inuid) {
    global $db;
    $strSQL = $controllerQuestoes->listaEscolasGremio($_REQUEST, true);
...
<form method="post" name="formularioEscolas" id="formularioEscolas" class="form form-horizontal">
    ...
    <input type="hidden" name="req" value="salvarEscolaGremio"/>
    <div class="ibox">
...
$(document).ready(function() {
	...
	if (!$.fn.DataTable.isDataTable('#formularioEscolas .tabela-listagem')) {
		var table = $('#formularioEscolas .tabela-listagem').DataTable({
            'data': dataSource,
            'columns': [
                { title: "<input type='checkbox' value='0' class='js-switch-txt superCheck ' />", "width": "5%" },
                { title: "Entidade" },
                { title: "Municipio" },
                { title: "Código Escola" },
                { title: "Nome Escola" }
			...
			'aoColumnDefs' : [{
                ...
                'render': function (data, type, full, meta) {
                    ...
                    return '<input type="checkbox" class="js-switch-txt ck-child " '+attrChecked+' co_entidade="' + co_entidade + '" qrpid="'+$("#qrpid").val()+'" perid="'+$("#perid").val()+'" name="id[]" value="' + co_entidade+ '">';
                }
            },{
                "targets": [ 1 ],
                "visible": false,
    ...
    $('.superCheck').on('click', function(){
        ...
        $('.ck-child', rows).prop('checked', checkedState);
    });
