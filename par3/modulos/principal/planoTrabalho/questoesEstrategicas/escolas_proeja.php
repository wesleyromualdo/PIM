<?php
$controllerQuestoes = new Par3_Controller_QuestoEsestrategicasEscolasProeja();
...
if ($qrpid && $inuid) {
    global $db;
    $strSQL = $controllerQuestoes->listaEscolasProeja($_REQUEST, true);
...
<form method="post" name="formularioEscolas" id="formularioEscolas" class="form form-horizontal">
    ...
    <input type="hidden" name="req" value="salvarEscolaProeja"/>
    <div class="ibox">
...
$(document).ready(function() {
    ...
    if (!$.fn.DataTable.isDataTable('#tb_render')) {
        var table = $('#tb_render').DataTable({
