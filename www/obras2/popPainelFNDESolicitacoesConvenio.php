...
<!--        <meta http-equiv="X-UA-Compatible" content="IE=EmulateIE9" /> -->
        ...
        <script type="text/javascript">
        		...
        		$('.ver-detalhes').click(function(){
            		a = window.open('popupSolicitacoesConvenioPainel.php?rstitem='+$('#esdid').val()+'&tpoid='+$('#tpoid').val()+'&estuf='+$(this).attr('estuf')+'&filtro='+$(this).attr('situacao'), 'Combopopup', "height=500,width=1500,scrollbars=yes,top=50,left=150" );
            		a.focus();
        		});

        		$('.label-danger').click(function(){
            		a = window.open('popupSolicitacoesConvenioPainel.php?rstitem='+$('#esdid').val()+'&tpoid='+$('#tpoid').val()+'&estuf='+$(this).attr('estuf')+'&filtro='+$(this).attr('situacao'), 'Combopopup', "height=500,width=1400,scrollbars=yes,top=50,left=150" );
            		a.focus();
        		});
    ...
    <body>
        <div class="row">
        	<div align="center" style="cursor:pointer; vertical-align: bottom;  margin-top: 1px; color: #fff; font-weight: bold;" class="titulo_box" >
                PAINEL DE SOLICITAÇÕES DE DESEMBOLSO CONVÊNIO
        	</div>
					...
					<form id="formPainel" name="formPainel" method="POST">
                        ...
                        $sql = "
                            SELECT
                                ...
                                FROM obras2.solicitacao_desembolso_convenio s

                                JOIN (SELECT
                                            MIN(obrid) obrid,
                                            pronumeroprocesso
                                        FROM obras2.vm_termo_convenio_obras
                                        GROUP BY pronumeroprocesso) as op ON op.pronumeroprocesso = s.processo

                                JOIN obras2.obras o ON o.obrid = op.obrid AND o.obridpai IS NULL  AND o.obrstatus = 'A'
                                JOIN workflow.documento d ON d.docid = s.docid AND d.esdid IN (1575, 1581, 1597)
