    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('.ver-detalhes').click(function(){
                //$('#div-entregas').load('popupDiligenciaPainelDentro.php?dlgitem='+$('#dlgitem').val()+'&tpdid='+$('#tpdid').val()+'&estuf='+$(this).attr('estuf'));
                //$('#myModal').modal();
                console.log(1);
                a = window.open('popupDiligenciaPainelDentro.php?&tpdid='+$('#tpdid').val()+'&tpoid='+$('#tpoid').val()+'&estuf='+$(this).attr('estuf')+'&filtro='+$(this).attr('situacao'), 'Combopopup', "height=500,width=1500,scrollbars=yes,top=50,left=150" );
                a.focus();
            });

            $('.label-danger').click(function(){
                a = window.open('popupDiligenciaPainelDentro.php?&tpdid='+$('#tpdid').val()+'&tpoid='+$('#tpoid').val()+'&estuf='+$(this).attr('estuf')+'&filtro='+$(this).attr('situacao'), 'Combopopup', "height=500,width=1400,scrollbars=yes,top=50,left=150" );
                a.focus();
            });
        ...
        <div class="col-md-12">
            <form id="formPainel" name="formPainel" method="POST">
                <div class="col-md-12"  style="float:left; margin-top: 15px; color: #fff; font-weight: bold;"  >
                    ...
                    <div class="col-md-4">
                        <div class="row">
                            <span>Tipo de diligência:</span>
                            <?php
                            $val = (!empty($_POST['tpdid'])) ? $_POST['tpdid'] : '';

                            $sql = " SELECT tpdid as codigo, tpddsc as descricao 
					                         FROM obras2.tipodiligencia
					                         WHERE tpdstatus = 'A' 
					                         ORDER BY tpdid";
                            $db->monta_combo("tpdid", $sql, "S", "Todos", "", "", "", "200", "N", "tpdid", false, $val, '', '', 'chosen');
                            ?>
                        </div>
                    </div>
                    ...
                    <div class="col-md-2">
                        <div class="row">
                            <input type="submit" name="pesquisar" class="btn" value="Pesquisar"/>
                        </div>
                    </div>
                ...
                if ($_POST['tpdid'] && $_POST['tpdid'] != ''){
                    $tipo = " AND td.tpdid = {$_POST['tpdid']} ";
//                    $tipoResp = " AND ufr.tpdid = {$_POST['tpdid']} ";
                }
                ...
                $sql = "
                            ...
                            LEFT JOIN(
                                ...
                                JOIN obras2.tipodiligencia td ON td.tpdid = dlg.tpdid AND td.tpdstatus = 'A'
                                JOIN obras2.obras o ON o.obrid = dlg.obrid AND o.obridpai IS NULL  AND o.obrstatus = 'A'
                                JOIN workflow.documento doc ON doc.docid = dlg.docid AND doc.esdid = ".ESDID_DILIGENCIA_AGUARDANDO_ANALISE_FNDE." 
                                JOIN entidade.endereco ende ON ende.endid = o.endid
                                JOIN territorios.municipio mun ON mun.muncod = ende.muncod
                                JOIN workflow.historicodocumento h ON h.hstid = doc.hstid AND h.aedid IN (SELECT aedid FROM workflow.acaoestadodoc WHERE esdiddestino = doc.esdid AND aedstatus = 'A')
