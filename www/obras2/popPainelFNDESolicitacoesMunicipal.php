        ...
        <script type="text/javascript">
            $(function(){
        		...
        		$('.ver-detalhes').click(function(){
            		a = window.open('popupSolicitacoesPainelMunicipal.php?rstitem='+$('#esdid').val()+'&tpoid='+$('#tpoid').val()+'&estuf='+$(this).attr('estuf')+'&filtro='+$(this).attr('situacao')+'&tslid='+$('#tslid').val(), 'Combopopup', "height=500,width=1500,scrollbars=yes,top=50,left=150" );
            		a.focus();
        		});

        		$('.label-danger').click(function(){
            		a = window.open('popupSolicitacoesPainelMunicipal.php?rstitem='+$('#esdid').val()+'&tpoid='+$('#tpoid').val()+'&estuf='+$(this).attr('estuf')+'&filtro='+$(this).attr('situacao')+'&tslid='+$('#tslid').val(), 'Combopopup', "height=500,width=1400,scrollbars=yes,top=50,left=150" );
            		a.focus();
        		});
    ...
    <body>
        <div class="row">
        	<div align="center" style="cursor:pointer; vertical-align: bottom;  margin-top: 1px; color: #fff; font-weight: bold;" class="titulo_box" >
        		PAINEL DE SOLICITAÇÕES MUNICIPAL
        	</div>
            ...
            <div class="col-md-12 box-principal">
                <div class="col-md-12">
					<form id="formPainel" name="formPainel" method="POST">
	                    <div class="col-md-12"  style="float:left; margin-top: 15px; color: #fff; font-weight: bold;" >
	                    	...
	                    	<div class="col-md-3">
	                    		<div class="row">
					                ...
					                <?php
					                $val = (!empty($_POST['tslid'])) ? $_POST['tslid'] : '';
	                    ...
	                    if ($_POST['tslid']){
							$tslid = " AND tss.tslid = ".$_POST['tslid'];
						}
                        ...
                        $sql = "
                            ...
                            LEFT JOIN(
                                SELECT
                                    mun.estuf,
                                    CASE WHEN DATE_PART('days', NOW() - COALESCE(h.htddata, d.docdatainclusao) ) < 15 THEN '1' END as verde,
                                    CASE WHEN (DATE_PART('days', NOW() - COALESCE(h.htddata, d.docdatainclusao)) >= 15 AND DATE_PART('days', NOW() - COALESCE(h.htddata, d.docdatainclusao)) <= 30) THEN '2' END as amarelo,
                                    CASE WHEN DATE_PART('days', NOW() - COALESCE(h.htddata, d.docdatainclusao)) > 30 AND DATE_PART('days', NOW() - COALESCE(h.htddata, d.docdatainclusao)) <= 60 THEN '3' END as vermelho,
                                    CASE WHEN DATE_PART('days', NOW() - COALESCE(h.htddata, d.docdatainclusao)) > 60 THEN '4' END as preto
                                FROM obras2.solicitacao s
                                JOIN obras2.tiposolicitacao_solicitacao tss ON (s.slcid = tss.slcid)
                                JOIN obras2.obras o ON o.obrid = s.obrid AND o.obridpai IS NULL  AND o.obrstatus = 'A'
                                JOIN workflow.documento d ON d.docid = s.docid AND d.esdid IN (" . ESDID_SOLICITACOES_AGUARDANDO_ANALISE . ", " . ESDID_SOLICITACOES_RETORNADO . ", " . ESDID_SOLICITACOES_AGUARDANDO_ANALISE_TI . ")
                        	 ...
                        	 WHERE hd.docid = d.docid
                                ...
                                WHERE s.slcstatus = 'A' AND tslid IN (1,2,3,4,5,8,13)
                                    $tipologia
                                    $item
                                    $tslid
                        	 ...
                        	 WHERE hd.docid = d.docid
                                ...
                                AND h.aedid IN (SELECT aedid FROM workflow.acaoestadodoc WHERE esdiddestino = d.esdid AND aedstatus = 'A')
                                RIGHT JOIN territorios.estado est ON est.estuf = mun.estuf
                                WHERE s.slcstatus = 'A' AND tslid NOT IN (1,2,3,4,5,8,13)
                                    $tipologia
                                    $item
                                    $tslid
                        ...
                        <div style="margin: 7px;">
                            ...
                            <span class="label">Solicitação de Desbloqueio do Cronograma, Exclusão de aditivos, Cancelamento de Obra e Desbloqueio de Conta Corrente, Alterar Fiscal de Obra, Gestor Unidade, Consulta Unidade (CGIMP)</span>
                        </div>
                        <div style="margin: 7px;">
                            <span class="label label-black" estuf="" situacao="P">maior que 61</span>
                            <span class="label label-danger" estuf="" situacao="R">de 31 até 60</span>
                            <span class="label label-warning" estuf="" situacao="A">de 16 até 30</span>
                            <span class="label label-success" estuf="" situacao="V">menor que 15</span>
                            <span class="label">Solicitação de Uso de Saldo, Alteração de Projeto/Serviço, Troca de Terreno, Alteração de Locação, Mobiliário e Prorrogação de prazo de Cumprimento de Objeto (CGEST)</span>
                        </div>
                    </form>
                </div>
            </div>
