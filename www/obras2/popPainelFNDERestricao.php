...
<!--        <meta http-equiv="X-UA-Compatible" content="IE=EmulateIE9" /> -->
        ...
        <script type="text/javascript">
				$(function(){
        		    ...
        		    //setTimeout(function(){$('#formPainel').submit();}, 50000);
        		$('.ver-detalhes').click(function(){
            		//$('#div-entregas').load('popupRestricoesPainel.php?rstitem='+$('#rstitem').val()+'&tprid='+$('#tprid').val()+'&estuf='+$(this).attr('estuf'));
        			//$('#myModal').modal();
        			console.log(1);
            		a = window.open('popupRestricoesPainel.php?rstitem='+$('#rstitem').val()+'&tprid='+$('#tprid').val()+'&tpoid='+$('#tpoid').val()+'&estuf='+$(this).attr('estuf')+'&filtro='+$(this).attr('situacao'), 'Combopopup', "height=500,width=1500,scrollbars=yes,top=50,left=150" );
            		a.focus();
        		});

        		$('.label-danger').click(function(){
            		a = window.open('popupRestricoesPainel.php?rstitem='+$('#rstitem').val()+'&tprid='+$('#tprid').val()+'&tpoid='+$('#tpoid').val()+'&estuf='+$(this).attr('estuf')+'&filtro='+$(this).attr('situacao'), 'Combopopup', "height=500,width=1400,scrollbars=yes,top=50,left=150" );
            		a.focus();
        		});
            ...
            <div class="col-md-12 box-principal">
                <div class="col-md-12">
					<form id="formPainel" name="formPainel" method="POST">
	                    <div class="col-md-12"  style="float:left; margin-top: 15px; color: #fff; font-weight: bold;"  >
	                    	...
	                    	<div class="col-md-3">
	                    		<div class="row">
					                ...
					                ?>
					                <input type="hidden" name="rstitem" id="rstitem" value="<?php echo (!empty($_POST['rstitem'])) ? $_POST['rstitem'] : 'T' ?>" />
					                <input type="radio" name="item_restrict" class="item_restrict" id="item_restrict_r" onclick="pegaItem(this.value)" value="R" <?php echo $select_r; ?> > Restrição
					                <input type="radio" name="item_restrict" class="item_restrict" id="item_restrict_i" onclick="pegaItem(this.value)" value="I" <?php echo $select_i; ?> > Inconformidade
					                <input type="radio" name="item_restrict" class="item_restrict" id="item_restrict_t" onclick="pegaItem(this.value)" value="T" <?php echo $select_t; ?> > Todas
				                </div>
	                    	</div>
	                    	...
	                    	<div class="col-md-3">
	                    		<div class="row">
					                ...
					                $sql = " SELECT tprid as codigo, tprdsc as descricao 
					                         ...
					                         WHERE tprstatus = 'A' 
					                         ORDER BY tprid";
					                $db->monta_combo("tprid", $sql, "S", "Todos", "", "", "", "200", "N", "tprid", false, $val, '', '', 'chosen');
					                ?>
				                </div>
                    		</div>
						        ...
						        <tr>
						            ...
						            <td>
						                ...
						                $sql = " SELECT tprid as codigo, tprdsc as descricao 
						                         ...
						                         WHERE tprstatus = 'A' 
						                         ORDER BY tprid";
						                $db->monta_combo("tprid", $sql, "S", "Todos", "pegaTipo(this.value)", "", "", "200", "N", "tprid", false, $val, '', '', 'chosen');
						                ?>
						            </td>
						            <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		                            ...
		                            <td>
		                                ...
		                                //$db->monta_combo("tpoid", $sql, "S", "Todas", "pegaTipologia(this.value)", "", "", 200, "N", "tpoid", false, $val, '', '','chosen');
		                                // $var,$sql,$habil,$titulo='',$acao,$opc,$txtdica='', $tamanho=4, $size='',$arrSelected = array(), $id = null,$return = false, $classe = null
		                                $db->monta_combo_multiplo("tpoid", $sql, "S",'','','', '', 1, '555',array(), "tpoid", false, 'chosen');
		                                ?>
		                            </td>
					            </tr>
	                    	</table -->
	                    ...
	                    if ($_POST['rstitem'] && $_POST['rstitem'] != 'T'){
							$item = " AND r.rstitem = '{$_POST['rstitem']}' ";
						}
	                    
						if ($_POST['tprid'] && $_POST['tprid'] != ''){
							$tipo = " AND tr.tprid = {$_POST['tprid']} ";
							$tipoResp = " AND ufr.tprid = {$_POST['tprid']} ";
						}
                        ...
                        $sql = "
                            ...
                            LEFT JOIN(
                                SELECT
                                    mun.estuf,
                                    CASE WHEN DATE_PART('days', NOW() - h.htddata) < 5 THEN '1' END as verde,
                                    CASE WHEN (DATE_PART('days', NOW() - h.htddata) >= 5 AND DATE_PART('days', NOW() - h.htddata) <= 10) THEN '2' END as amarelo,
                                    CASE WHEN (DATE_PART('days', NOW() - h.htddata) > 10 AND DATE_PART('days', NOW() - h.htddata) <= 30) THEN '3' END as vermelho,
                                    CASE WHEN DATE_PART('days', NOW() - h.htddata) >= 30 THEN '4' END as preto
                                FROM obras2.restricao r
                                JOIN obras2.tiporestricao tr ON tr.tprid = r.tprid AND tr.tprstatus = 'A'
                                JOIN obras2.obras o ON o.obrid = r.obrid AND o.obridpai IS NULL  AND o.obrstatus = 'A'
                                ...
                                JOIN workflow.historicodocumento h ON h.hstid = d.hstid AND h.aedid IN (SELECT aedid FROM workflow.acaoestadodoc WHERE esdiddestino = d.esdid AND aedstatus = 'A')
	                                    ...
	                                    <div class=\"panel-body panel-body-".$class."\" estuf=\"".$estado['estuf']."\">
	                                        ...
	                                        <div style=\"margin-top: 14px;text-align: center;\" align=\"center\">";
	                                            if(true){
	                                            	...
	                                            	$html .= "<span class=\"label label\" style=\"color: black;\">&nbsp;</span>";
	                                            }
                            $html .= "
	                                        </div>
	                                        <div class=\"clearfix\"></div>
