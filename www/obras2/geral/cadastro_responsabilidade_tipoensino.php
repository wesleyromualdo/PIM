...
function listaOrgao(){
	...
	if ($db->testa_superuser()){		
		$sql = "SELECT
				...
				WHERE
					orgstatus = 'A'";
		$orgaoExistentes = $db->carregar($sql);
		...
		if( $db->pegaUm( $sql ) > 0 ){
			...
			$sql = "SELECT
					...
					FROM
						 obras2.orgao o
					INNER JOIN  obras2.usuarioresponsabilidade ur ON ur.orgid = o.orgid
					WHERE
...
function atribuiOrgao($usucpf, $pflcod, $orgid){
	...
	if ( $orgid[0] ){
		foreach ($orgid as $valor){
			...
			$sql = "INSERT INTO  obras2.usuarioresponsabilidade
...
function buscaOrgaoCadastrado($usucpf, $pflcod){
	...
	$sql = "SELECT DISTINCT 
			...
			INNER JOIN  obras2.orgao o ON ur.orgid = o.orgid 
			WHERE 
				ur.rpustatus='A' AND 
