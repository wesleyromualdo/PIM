...
function listaEstados(){
	...
	$sql = "SELECT
			...
			ORDER BY 
				estuf, estdescricao";
	$estadosExistentes = $db->carregar($sql);
	...
	if (is_array($estuf) && $estuf[0]){
		...
		for ($i = 0; $i < $count; $i++){
			...
			$sql = "INSERT INTO obras2.usuarioresponsabilidade 
						(estuf,usucpf,rpustatus,rpudata_inc,pflcod)
					VALUES
						('{$estuf[$i]}','{$usucpf}','A','{$data}','{$pflcod}')";
			
			$sql_insere = $db->carregar($sql);
