...
require_once "config.inc";
	...
	function montaElementoPorTarefa( Tarefa $objTarefa )
	{
		...
		$objElemento[ 'previsto']			= $objTarefa->getPrevisaoMeta();
		$objElemento[ 'realizado']			= $objTarefa->getQuantidadeRealizada();
		$objElemento[ 'gasto']				= '1000';
		$objElemento[ 'porcentagem']		= $objTarefa->getQuantidadeRealizadaPercentual();
	...
	function montaGantt( $strTitulo , $subTitulo , $arrElementos , $dataLimiteInicioTimestamp , $dataLimiteFimTimestamp )
	{
		...
		else if ( $intPeriodo < 8 * $intMes )
		{
			$arrEscalas[ 'dias' ] = DAYSTYLE_ONELETTER;
			$arrEscalas[ 'meses' ] = MONTHSTYLE_LONGNAMEYEAR4;			
						
		}
		else if ( $intPeriodo <= 1*$intAno )
		{
	...
	switch( $strClasseContainer )
	{
		case 'Projeto':
		{
			...
			else
			{
				...
				$dataLimiteInicio =  $objTarefa->getDataInicioTimestamp();
				$dataLimiteFim =  $objTarefa->getDataFimTimestamp();
			}
			break;
		}
		case 'Acao':
		{
			...
			else
			{
				$objTarefa = new TarefaAcao();
				$objTarefa = $objTarefa->getTarefaPeloId( $intTarefaId );
				$dataLimiteInicioTimestamp =  $objTarefa->getDataInicioTimestamp();
				$dataLimiteFimTimestamp =  $objTarefa->getDataFimTimestamp();
			}
			break;
		}
		default:
