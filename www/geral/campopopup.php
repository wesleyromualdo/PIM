...
//		$dados_combo['sql'] = str_replace("WHERE","WHERE upper(removeacento( ".$dados_combo['whereAuto']." )) like upper(removeacento('%".($q)."%')) AND ", $dados_combo['sql']);
		$sql = "SELECT
					codigo, descricao
				FROM (".$dados_combo['sql'].") foo
				WHERE
					upper(removeacento( descricao )) like upper(removeacento('%".($q)."%'))";
...
//		ver($sql);
		if(count($arDados)>0){
			foreach ($arDados as $key=>$value){
				if (strpos(strtolower(removeAcentos($value['descricao'])), strtolower(removeAcentos(($q)))) !== false) {
	...
	$_POST['filtro'] = $_POST['filtro'] ? $_POST['filtro'] : $_SESSION['indice_sessao_campo_popup'][$_REQUEST['nome']]['filtrobkp'];
	if ( is_array($_POST['filtro']) ){
		$_SESSION['indice_sessao_campo_popup'][$_REQUEST['nome']]['filtrobkp'] = $_POST['filtro'];
