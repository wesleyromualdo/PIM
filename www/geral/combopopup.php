...
if (is_array($_POST['filtro'])) {
	...
	foreach ($_POST['filtro'] as $k => $val) {
		${'filtro[' . $k . ']'} = $val;
		if (trim($val) != '') {
			$where[] = $_POST['numeric'][$k] ? "$k = '" . intval($val) . "'" : "public.removeacento($k) ilike public.removeacento( '%$val%' )";
		}
	}
...
if (isset($_REQUEST['pegar_dados_item']) == true) {
	...
	if (strpos(strtolower($sql), 'where') !== false) {
		$sql = str_replace('WHERE', 'where ' . $where . ' and ', $sql);
	} else if (strpos(strtolower($sql), 'group by') !== false) {
		$sql = str_replace('GROUP BY', 'where ' . $where . ' group by ', $sql);
	} else if (strpos(strtolower($sql), 'order by') !== false) {
		$sql = str_replace('ORDER BY', 'where ' . $where . ' order by ', $sql);
	} else {
...
$_REQUEST['ordemCampo'] = !$_REQUEST['ordemCampo'] && is_array($arrOrdem) ? $arrOrdem[0] : $_REQUEST['ordemCampo'];

$ordemCampo = $_REQUEST['ordemCampo'] == 'cod' ? 'cod' : 'dsc';
...
<html>
    <head>
        ...
        <!--
            ...
            <?php if (is_array($codigos_fixos)): ?>
	            <?php foreach ($codigos_fixos as $codigo_fixo): ?>
	                <?php if ($codigo_fixo): ?>
             ...
             * @return void
             */
            function combo_popup_altera_item_radio(cod, descricao, obRadio) {
                ...
                if (fixo == false) {
                    window.opener.combo_popup_remover_item_radio(nome_popup, cod, true);
...
if (count($registros)):
	...
	foreach ($registros as $registro):
		$descricao = simec_htmlentities(trim($registro['descricao']));
		//$descricao =  $registro['descricao'] ;
		//$arrVisivel
		//$arrOrdem

		$onclick = 'javascript:combo_popup_altera_item( \'' . $registro['codigo'] . '\', \'' . $descricao . '\', this ); ' . $funcaoJS;
		$onclickRadio = 'javascript:combo_popup_altera_item_radio( \'' . $registro['codigo'] . '\', \'' . $descricao . '\', this ); ' . $funcaoJS;
		?>
				                            <tr bgcolor="#<?=$posicao % 2 == 0 ? 'f4f4f4' : 'e0e0e0'?>">
				                                <td width="1">
				                                    <?if ($maximo == 1): ?>
				                                        <input descricao="<?=$descricao?>" name="combo_popup_checkbox" type="radio" id="combo_popup_checkbox_<?=$registro['codigo']?>" value="<?=$registro['codigo']?>" onclick="<?=$onclickRadio?>"/>
				                                    <?else: ?>
