...
//			$endorigem =  "../../arquivos/".$orig."/".floor($f['arqid']/1000)."/".$f['arqid'];
			$endorigem =  '../../arquivos/'.(($_REQUEST["_sisarquivo"])?$_REQUEST["_sisarquivo"]:$_SESSION["sisarquivo"]).'/'. floor($f['arqid']/1000) .'/'.$f['arqid'];
...
//        ver(file_exists('../../arquivos/par/arquivosDemanda/15/2798677.jpg') , file_exists($endorigem) , $endorigem , $orig , $files,d);
	return $fzip;
}
...
//    ver($_SESSION['downloadfiles'] , $files,d);
		$filezip = processararquivos($files,$_SESSION['downloadfiles']['pasta']['origem'],$_SESSION['downloadfiles']['pasta']['destino']);
		if(count($filezip) > 0) {
			$files = $files[0];
