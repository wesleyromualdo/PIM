...
$dadosfisicoporsemestre[0] = 0;
$dadosfisicoporsemestre[1] = 0;
foreach($dadosfisicopormes as $k=>$v){
	if($k<=5){
		$dadosfisicoporsemestre[0] += $v;
	}else if($k>5){
		$dadosfisicoporsemestre[1] += $v;
	}
}

ksort($dadosfisicopormes);
ksort($dadosfisicoporsemestre);

$sql = "SELECT prgcod, acacod, unicod, loccod FROM monitora.acao WHERE acaid = ".$_REQUEST['acaid'];

$dadosFiltro = $db->pegaLinha($sql);

// carrega os dados financeiros
$sql = "SELECT 
			rofmes, 
			max(rofdata) as dataatu, 
		...
		FROM 
			financeiro.execucao as exe
		WHERE 
			prgcod = '{$dadosFiltro['prgcod']}' and 
			acacod = '{$dadosFiltro['acacod']}' and 
			unicod = '{$dadosFiltro['unicod']}' and 
			loccod = '{$dadosFiltro['loccod']}' and 
...
$mesmaximo = 1;
$dadofinanceiroporsemestre[0] = 0;
$dadofinanceiroporsemestre[1] = 0;
...
// 		$dadofinanceiropormes[((integer) $linhafin['rofmes'] - 1)] = (integer) $linhafin['rofliquidado_favorecido'];
		$dadofinanceiropormes[((integer) $linhafin['rofmes'] - 1)] = (integer) $linhafin['empenhado'];
		
		if($linhafin['rofmes'] <= 6){
			$dadofinanceiroporsemestre[0] += $linhafin['empenhado'];
		}else if($linhafin['rofmes'] > 6){
			$dadofinanceiroporsemestre[1] += $linhafin['empenhado'];
		}
...
ksort($dadofinanceiroporsemestre);

//dbg($dadofinanceiropormes,1);
// ver($dadosfisicoporsemestre, $dadosfisicopormes);
// exit;
...
$a = new LinePlot( $dadosfisicoporsemestre );
...
$b = new LinePlot( $dadofinanceiroporsemestre );
...
$grafico->xaxis->SetFont( FF_VERDANA, FS_NORMAL, 8 );
$grafico->xaxis->SetTitle( 'Semestre', 'center' );
