...
<script>
    ...
    /*
     *  Chamada para a validação de documentos SIMEC
     */
    $('#validar-documento-simec').click(function () {
        var validacao = true;
        if ($('#nudoc').val() == '' || $('#nudoc').val().length < 19) {
            mensagem = '\n A chave informada não é válida.';
            validacao = false;
        }
        if (!validacao) {
            alert(mensagem);
...
<!-- Insere parametros no link do botão "Solicitar acesso", para mudar o background da tela de cadastro também. -->
<?php
