...
var pathToImages = '../includes/JsLibrary/date/displaycalendar/images/';
...
function displayCalendar(inputField,format,buttonObj,displayTime,timeInput,strAttribute)
{
	if(!strAttribute) strAttribute = 'value';

	if(displayTime)calendarDisplayTime=true; else calendarDisplayTime = false;
	if(inputField.value.length>0){
		
		if(!format.match(/^[0-9]*?$/gi)){
			var items = inputField[strAttribute].split(/[^0-9]/gi);
			...
			currentMonth = inputField[strAttribute].substr(monthPos,2)/1 -1;	
			var yearPos = format.indexOf('yyyy');
			currentYear = inputField[strAttribute].substr(yearPos,4);		
			var dayPos = format.indexOf('dd');
			tmpDay = inputField[strAttribute].substr(dayPos,2);		
			
			var hourPos = format.indexOf('hh');
			if(hourPos>=0){
				tmpHour = inputField[strAttribute].substr(hourPos,2);	
				currentHour = tmpHour;
			}else{
				currentHour = '00';
			}
			...
			if(minutePos>=0){
				tmpMinute = inputField[strAttribute].substr(minutePos,2);	
