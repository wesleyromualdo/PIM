function Questionario(param){
	...
	//  Comentei para não desabilitar e reabilitar nos questionarios desabilitados!!	
	//	desabilitaForm(true);
...
//						desabilitaForm(false);			    
	...
	this.salvar = function (perid){
		...
		if( jQuery(':file[class!="dz-hidden-input"]').length > 0 ){
			var erro = false;
			var submit = false;
			jQuery(':file[class!="dz-hidden-input"]').each(function(){
				var name = jQuery(this).attr('name').split('_');
				var perid_quest = name[1];
				var perid_pai = jQuery('#perid_pai_'+perid_quest).val();
				var itpid_pai = jQuery('#itpid_pai_'+perid_quest).val();
				var value = jQuery(this).val();
				if(jQuery('#linha_'+perid_pai+'_'+itpid_pai).css('display') != 'none'){
					if(value == ''){
						erro = true;
					}else{
						submit = true;
					...
					jQuery('#linha_'+perid_pai+'_'+itpid_pai).empty();
				}
			});
			if(erro){
...
//			if( jQuery(':file').attr("style") != "display: none;" && jQuery(':file').attr("class") != 'dz-hidden-input' ){
//				if(jQuery(':file').val()){
//				} else {
//					alert('Você precisa Anexar um arquivo.');
//					divCarregado();
	...
	function desabilitaForm(param){
		jQuery('#formulario select,textarea,input').each(function (){
			if(jQuery(this).attr("name") != "csFiltroArvore"){
				if ( (jQuery(this).val().indexOf('Próximo') > -1 && !bntProx) 
					 || 
					 (jQuery(this).val().indexOf('Anterior') > -1 && !bntAnt) 
				   	 || 
				   	 (jQuery(this).val().indexOf('Anterior') == -1 && jQuery(this).val().indexOf('Próximo') == -1) ){
