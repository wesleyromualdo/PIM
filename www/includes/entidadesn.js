...
function abreMapaEntidade(tipoendereco){
	...
	var entid = '';
	if (window.document.getElementById("entcodent")){
		entid = window.document.getElementById("entcodent").value;
	}
	
	var janela=window.open('../apigoogle/php/mapa_padraon.php?tipoendereco='+tipoendereco+'&longitude='+longitude+'&latitude='+latitude+'&polo='+pololatitude+'&poloLong='+pololongitude+'&entid='+entid, 'mapa','height=650,width=570,status=no,toolbar=no,menubar=no,scrollbars=no,location=no,resizable=no').focus();
...
function carregaDadosEntidadeJuridica(dados) {
	...
	if(dados[38]) document.getElementById('entrazaosocial').value = dados[38];
...
function carregaDadosEntidadeFisica(dados) {
	...
	if(dados[2]){
		var comp = new dCPF();
		comp.buscarDados(dados[2]);
		
		if(comp.dados.no_pessoa_rf)
			dados[3] = comp.dados.no_pessoa_rf;
	}
		...
		if(entidadesl[i]) {
			...
			if(enderecos[0] != "naoendereco") {
				for(j=0;j<enderecos.length;j++) {
					...
					html += "<tr><td class='SubTituloDireita'><strong>Endereço-"+(j+1)+":</strong></td><td>"+dadosendereco[7]+", "+dadosendereco[13]+", "+dadosendereco[2]+", "+dadosendereco[5]+", "+dadosendereco[3]+", Número "+dadosendereco[8]+"</td></tr>";
				}
			}
...
function getEntidadePeloCPF(cpf) {
	if(cpf) {
		...
		displayStaticMessage("<p align=center>Carregando...</p>","");
		var myAjax = new Ajax.Request('/geral/consultadadosentidade.php?requisicao=pegarentidadePorcpfcnpj&entnumcpfcnpj=' + cpf + "&funidEspecifico=" + funidEspecifico + "&funidFiltroEspecifico=" + funidFiltroEspecifico,
			{
				method: 'post',
				asynchronous: false,
				onComplete: function(resp) {
	...
	var myAjax = new Ajax.Request('/geral/consultadadosentidade.php?requisicao=pegarentidadePorCnpj&entnumcpfcnpj=' + cnpj +'&entnome='+comp.dados.no_empresarial_rf,
			{
				...
				onComplete: function(resp) {
					if(resp.responseText == "naoexiste") {
						...
						if(document.getElementById('entrazaosocial')) {
