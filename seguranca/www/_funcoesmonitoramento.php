...
function monitoramentoGRID($dados) {
	...
	if($_SISTEMAS_EX) {
		foreach($_SISTEMAS_EX as $monsisdiretorio => $monsisdsc) {
			$sistemas[] = array('sisid' => $monsisdiretorio, 'sisdsc' => $monsisdsc);
		}
	}
	...
	if($sistemas[0]) {
		...
		foreach($sistemas as $sis) {
			...
			if($tipomonitoramento[0]) {
				foreach($tipomonitoramento as $tpm) {
					unset($vls,$args,$style);
					if($tpm['tmoparametros']) {
						$args = explode(";",$tpm['tmoparametros']);
						if($args[0]) {
							$vls = explode(":",$args[0]);
							if($_GRID[$sis['sisid']][$tpm['tmoid']] <= $vls[0]) $style="style=\"background-color:".$vls[1].";\" title=\"{$vls[2]}\"";
						}
						if($args[1]) {
							$vls = explode(":",$args[1]);
							if($_GRID[$sis['sisid']][$tpm['tmoid']] >= $vls[0]) $style="style=\"background-color:".$vls[1].";\" title=\"{$vls[2]}\"";
					...
					$HTML[$sis['sisid']] .= "<td align='right' {$style}>".(($_GRID[$sis['sisid']][$tpm['tmoid']])?"<b>".$_GRID[$sis['sisid']][$tpm['tmoid']]."</b>":"0")."</td>";		
				}
			} else {
...
function pegarDadosPorPagina($dados) {
	...
	$resultado = $db->carregar("SELECT COUNT(e.mnuid) as num,  to_char(estdata, 'DD') as dia  FROM estatistica.estatistica_".sprintf("%04d_%02d", $dados['ano'], $dados['mes'])." e 
	...
	$resultado = $db->carregar("SELECT COUNT(DISTINCT u.usucpf) as num,  to_char(estdata, 'DD') as dia FROM estatistica.estatistica_".sprintf("%04d_%02d", $dados['ano'], $dados['mes'])." e
								INNER JOIN seguranca.menu m ON m.mnuid=e.mnuid 
	...
	$resultado = $db->carregar("SELECT ROUND(CAST(AVG(estmemusa) as numeric),2) as num, to_char(estdata, 'DD') as dia FROM estatistica.estatistica_".sprintf("%04d_%02d", $dados['ano'], $dados['mes'])." e 
						 		INNER JOIN seguranca.menu m ON m.mnuid=e.mnuid
	...
	$resultado = $db->carregar("SELECT ROUND(CAST(AVG(esttempoexec) as numeric),2) as num, to_char(estdata, 'DD') as dia FROM estatistica.estatistica_".sprintf("%04d_%02d", $dados['ano'], $dados['mes'])." e 
						 		INNER JOIN seguranca.menu m ON m.mnuid=e.mnuid
								WHERE e.sisid='".$dados['sisid']."' AND (date_part('year',estdata)::varchar||date_part('month',estdata)::varchar)::varchar='".$dados['ano'].(integer)$dados['mes']."' AND m.mnulink ILIKE '%".$dados['link']."%' 
								GROUP BY to_char(estdata, 'DD') ORDER BY to_char(estdata, 'DD')");
