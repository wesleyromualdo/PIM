...
{
    if ($1_11(this).val() == 1) {
    	$1_11('#estuf_acessorapido', tab).parents('tr:first').show();
        $1_11('#muncod_acessorapido', tab).parents('tr:first').hide();
        $1_11('#muncod_acessorapido', tab).find('option').prop('selected', false);
...
//         $1_11('#estuf_acessorapido', tab).trigger('chosen:updated');
    	$1_11('#estuf_acessorapido', tab).parents('tr:first').show();
    	$1_11('#muncod_acessorapido', tab).parents('tr:first').show();
    }
});
...
{	
	...
	aplicarFiltroAcessoRapido(dados);
});
