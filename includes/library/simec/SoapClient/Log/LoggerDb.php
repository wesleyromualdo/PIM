<?php
/**
 * $Id: LoggerDb.php 117540 2017-01-16 13:53:38Z lucasgomes $
 */
...
class Simec_SoapClient_Log_LoggerDb extends Simec_SoapClient_Log_Logger
{
    ...
    public function writeLog(array $data)
    {
        $insStmt = sprintf(
            $this->insertStmt,
            addslashes($data['requestContent']),
            addslashes($data['requestHeader']),
            $data['requestTimestamp'],
            addslashes($data['responseContent']),
            addslashes($data['responseHeader']),
        ...
        if ($this->db instanceof cls_banco) {
            $this->lastid = $this->db->pegaUm($insStmt);
            $this->db->commit();
        } else {
...
) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %s) returning lwsid
DML;
    }
