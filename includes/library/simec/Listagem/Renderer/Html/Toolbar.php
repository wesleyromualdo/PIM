 ...
 *
 * @version $Id: Toolbar.php 142588 2018-08-10 17:48:47Z danielfiuza $
 * @filesource
 */
...
class Simec_Listagem_Renderer_Html_Toolbar
{
    ...
    /**
     * Exporta o conteúdo listado no formato PDF.
     */
    const EXPORTAR_PDF = 9;
    ...
    public function render()
    {
        ...
        $html .= <<<HTML
<nav class="hidden-xs navbar navbar-default navbar-listagem {$className} hidden-xs" role="navigation" style="margin-top: 0px;float: right;width: 500px;">
HTML;
        $itens = $this->itens;
            ...
            foreach ($itens as $item) {
                switch ($item) {
                    ...
                    case self::EXPORTAR_XLS:
                        ...
                        break;
                    case self::EXPORTAR_PDF:
                        $html .= $this->renderExportarPdf();
                        break;
...
HTML;
    ...
    <script >
        ...
        $('[data-toggle=popover]').on('shown.bs.popover', function () {
          $(this).find('.popover').css('top',parseInt($('.popover').css('top')) + 22 + 'px')
        });
...
HTML;
    ...
    protected function renderAdicionar()
    {
        ...
        return <<<HTML
        <button type="button" 
                class="btn btn-default btn-adicionar" 
                data-title='Ajuda - Adicionar'
...
HTML;
    ...
    protected function renderExportarPdf()
    {
        $id = $this->config->getId();

        return <<<HTML
        <button type="button" class="btn btn-default btn-pdf" data-title='Ajuda - Exportar PDF'
                data-toggle="popover"
                data-trigger="hover"
                data-placement="top"
