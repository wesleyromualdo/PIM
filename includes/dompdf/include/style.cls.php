...
//      $val = preg_replace_callback("/([0-9]+) (pt|px|pc|em|ex|in|cm|mm|%)/S", "\\1\\2", $val);
      $val = strtr($val,array("/([0-9]+) (pt|px|pc|em|ex|in|cm|mm|%)/S"=> "\\1\\2"));
    }
...
//    $val = strtr(array("/\s*\,\s*/"=> ","), $val); // when rgb() has spaces
    $arr = explode(" ", $val);
...
//      $val = preg_replace("/url\(['\"]?([^'\")]+)['\"]?\)/","\\1", trim($val));
      $val = strtr(trim($val),array("/url\(['\"]?([^'\")]+)['\"]?\)/"=>"\\1"));
...
//    $tmp = preg_replace("/\s*\,\s*/", ",", $val); // when rgb() has spaces
    $tmp = strtr($val,array("/\s*\,\s*/"=>",")); // when rgb() has spaces
    $tmp = explode(" ", $tmp);
  ...
  protected function _set_border($side, $border_spec, $important) {
    $border_spec = strtr($border_spec,array("/\s*\,\s*/"=>","));
...
//    $val = preg_replace("/\s*\,\s*/", ",", $val); // when rgb() has spaces
    $val = strtr($val,array("/\s*\,\s*/"=> ",")); // when rgb() has spaces
    $arr = explode(" ", $val);
    foreach ($arr as $value) {
      $value = trim($value);
      if ( in_array($value, self::$BORDER_STYLES) ) {
