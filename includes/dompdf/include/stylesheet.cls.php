...
//    }
  ...
  protected function _selector_an_plus_b($expr, $last = false) {
    $expr = preg_replace_callback("/\s/", function($matches){return $matches[1];}, $expr);
  ...
  function apply_styles(Frame_Tree $tree) {
    ...
    foreach ($tree->get_frames() as $frame) {
      ...
      if ( ($str = $frame->get_node()->getAttribute("style")) !== "" ) {
        ...
        $str = preg_replace_callback("'/\*.*?\*/'si", function($matches){return $matches[1];}, $str);
  ...
  private function _parse_css($str) {
    ...
    $css = preg_replace_callback(array(
      "'/\*.*?\*/'si", 
      "/^<!--/",
      "/-->$/"
    ), function($matches){return $matches[1];}, $str);
