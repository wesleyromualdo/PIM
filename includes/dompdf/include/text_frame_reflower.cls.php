...
//       $text = ltrim($text, " \n\r\t");
    return preg_replace_callback(self::$_whitespace_pattern, function($matches){return $matches[1];}, $text);
  }
...
//     pre_r("Offset: " . $offset);
  ...
  protected function _layout_line() {
    ...
    case "pre-line":
      ...
      $frame->set_text( $text = preg_replace_callback( "/[ \t]+/u", function($matches){return $matches[1];}, $text ) );
...
//     }
  ...
  function get_min_max_width() {
    ...
    default:
    case "normal":
      $str = preg_replace_callback(self::$_whitespace_pattern,function($matches){return $matches[1];}, $str);
    case "pre-wrap":
    case "pre-line":
    ...
    case "normal":
    case "nowrap":
      $str = preg_replace_callback(self::$_whitespace_pattern,function($matches){return $matches[1];}, $text);
      break;

    case "pre-line":
      //XXX: Is this correct?
      $str = preg_replace_callback( "/[ \t]+/u", function($matches){return $matches[1];}, $text);
