...
class XLSXWriter
{
	...
	public function writeSheet(array $data, $sheet_name='', array $header_types=array() )
	{
		...
		if (!empty($header_row))
		{
			...
			{
				$this->writeCell($file, 0, $k, $v, $cell_format='string');
			}
			$file->write('</row>');
		}
	...
	{
		file_put_contents("php://stderr", date("Y-m-d H:i:s:").rtrim(is_array($string) ? simec_json_encode($string) : $string)."\n");
	}
	//------------------------------------------------------------------
	...
	{
		return str_replace("'", "&#39;", simec_htmlspecialchars($val));
	}
	//------------------------------------------------------------------
