...
class BackTraceExplain
{
		...
		foreach ( $aArgument as $mArgument )
		{
			switch( gettype( $mArgument ) )
			{
				...
				case 'NULL':
					...
					break;
				case 'object':
					$sSummary = 'object( ' . simec_htmlentities( get_class( $mArgument ) ) . ' )';
					break;
				case 'resource':
					$sSummary = 'resource( ' . simec_htmlentities( strstr( (string) $mArgument, '#' ) . ' ' . get_resource_type( $mArgument ) ) . ' )';
					break;
				case 'string':
					...
					$sSummary = simec_htmlentities( '"' . $mArgument . '"' );
					break;
				default:
					$sSummary = 'unknown type ' . simec_htmlentities( gettype( $mArgument ) );
					break;
			}
	...
	protected function fetchCall()
	{
		...
		return simec_htmlentities( $sClass . $sType . $sFunction ) . '(' . $sArgument . ')';
	}
	...
	{
		$sLocation = simec_htmlentities( $this->oBackTrace->getFile() ) . ' on line ' . $this->oBackTrace->getLine();
