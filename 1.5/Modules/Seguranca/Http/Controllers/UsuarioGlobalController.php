...
class UsuarioGlobalController extends Controller
{
    ...
    {  
        ...
        return view('seguranca::usuarioglobal.index', compact('unidadesFederativas','sistema', 'perfis', 'unidadesOrcamentarias', 'cargos', 'prefixo'));
    }
    ...
    public function create()
    {
        ...
        return view('seguranca::usuarioglobal.criar', ['sisid' => $sisid, 'prefixo' => $this->prefixo]);
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route($this->prefixo . '.usuarioglobal.index')->with('success', 'Usuario salvo com sucesso');
    }
    ...
    public function edit($id)
    {
        ...
        $unidadesGestoras = UnidadeGestora::where('ungstatus','=','A')->where('muncod','=',$usuario->muncod)->orderBy('ungcod')->orderBy('ungdsc', 'ASC')->pluck('ungdsc', 'ungcod');
        
        $sistema =  $_SESSION['sisdsc'];
        
        $perfis = Perfil::where('sisid', '=', $_SESSION['sisid'])->orderBy('pfldsc', 'ASC')->pluck('pfldsc', 'pflcod');
        
        $cargos = Cargo::lists('cardsc','carid');
        ...
        return view('seguranca::usuarioglobal.editar', compact('abaUser', 'abaUserSis', 'unidadesFederativas','sistema', 'perfis', 'unidadesOrcamentarias', 'cargos', 'usuario','unidadesGestoras','tipoOrgao','municpiosPorUnidade', 'prefixo'));
    ...
    public function update(UsuarioRequest $request, $usucpf)
    {
        ...
        return redirect()->route($this->prefixo . '.usuarioglobal.index')->with('success', 'Usuario atualizado com sucesso');
    }
    
    public function destroy($id)
    {
        Usuario::find($id)->update(['pflstatus' => 'I']);        
        return redirect()->route($this->prefixo . '.usuarioglobal.index')->with('success', 'Usuario excluido com sucesso');
    }    
...
//                                //->orWhere('seguranca.usuario_sistema.pflcod', '=', $pflcod);
            ...
            $datatables = Datatables::of($usuarios)
                    ->addColumn('action', function ($usuario) {
                        $url_edit = route($this->prefixo . ".usuarioglobal.edit", $usuario->usucpf);
                        return <<<HTML
